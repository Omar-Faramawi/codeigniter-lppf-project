...
class News extends CI_Controller {
	...
	public function index()
	{
		...
		$row = array();
		$row['heading'] = $this->newsModel->get_news_heading();
		$row['sections'] = $this->newsSectionsModel->get_all_sections();
		$this->load->view('header', array('title' => "News"));	
	...
	function item($id){
		...
		$this->load->model('news_items_model', 'newsSectionsModel');
		$row = array();
		$row['newsItem'] = $this->newsSectionsModel->getItem($id);
		$row['heading'] = $this->newsModel->get_news_heading();
		$this->load->view('header', array('title' => "News"));	
		$this->load->view('newsitem', array('row' => $row));	
		$sidebarArray = array();
		$this->load->model('social_links_model', 'socialLinks');
		$sidebarArray['links'] = $this->socialLinks->getLinks();
